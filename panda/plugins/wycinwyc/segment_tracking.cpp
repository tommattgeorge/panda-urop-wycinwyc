...
bool enable_segment_tracking(void* self, panda_cb pcb){
    pcb.phys_mem_before_read =(void (*)(CPUState*, target_ptr_t, target_ptr_t, size_t)) phys_mem_read_segment_cb;
    panda_register_callback(self, PANDA_CB_PHYS_MEM_BEFORE_READ, pcb);

    pcb.phys_mem_before_write =(void (*)(CPUState*, target_ptr_t, target_ptr_t, size_t, uint8_t*)) phys_mem_write_segment_cb;
